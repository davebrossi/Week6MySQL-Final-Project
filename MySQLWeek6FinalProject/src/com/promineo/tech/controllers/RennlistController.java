...
import com.promineo.tech.Models.RennlistViewModel;
...
public class RennlistController implements IRennlistController {
	private RennlistViewModel viewModel;
	private UserService userService;
	private ForumService forumservice;
	private Forum_Post_Service forum_post_Service;

	public RennlistController(RennlistViewModel model) {
		viewModel = model;
		userService = new UserService();
		forumservice = new ForumService();
		forum_post_Service = new Forum_Post_Service();
	}
	...
	public void StartRennlistService() {
		...
		case FORUM_POST:
			switch (viewModel.CrudChoice) {
			case CREATE:
				forum_post_Service.createForum_Post(new Forum_Post(0, viewModel.postTitle, viewModel.postBody));
				System.out.println("Forum Post has been submitted");
				break;
			case READ:
				System.out.println("List of active forum threads.");
				ArrayList<Forum_Post> forum_posts = forum_post_Service.getForum_Post();
				for (Forum_Post c : forum_posts) {
					System.out.println(c);
				}
				break;
			case UPDATE:
				forum_post = forum_post_Service.getForum_Post(viewModel.forum_post_ID);
				if (forum_post != null) {
					forum_post.setForum_Post_Name(viewModel.postTitle);
					forum_post.setForum_Post_Body(viewModel.postBody);
					forum_post_Service.updateForum_Post(forum_post);
			...
			case DELETE:
				forum_post = forum_post_Service.getForum_Post(viewModel.forum_post_ID);
				if (forum_post != null) {
					forum_post_Service.deleteForum_Post(forum_post);
